...
def quality_over_time_comparison(results, labels, num_segments=constants.N_SEGMENTS):
    ...
    for i in range(len(results)):
        label = labels[i]
        res = results[i]
        color = colors[label]  # Use color from the dictionary

        # Customize the plotting for "cross_entropy"
        if label == "cross_entropy":
            plt.plot(
                res.quality_history,
                marker='o',
    ...
    plt.title("Quality Level Chosen per Segment by Policy")
    plt.xlabel("Segment #")
...
def quality_choice_hist_comparison(simulation_results, labels):
    ...
    for i, label in enumerate(labels):
        ...
        plt.bar(x + i * width, heights, width=width, label=label, color=colors[label])
...
def compare_co2_hist(results, labels, num_segments=constants.N_SEGMENTS):
    ...
    plt.bar(labels, emissions, color=[colors[label] for label in labels])
    plt.ylabel("Total CO₂ Emitted (g)")
    plt.title("Simulated CO2 emissions per-policy")
...
def plot_four_objective_radar(results, labels):
    ...
    inv_rebuffers = np.array([1 / r.total_rebuffer_time if r.total_rebuffer_time > 0 else 1e6 for r in results])
    inv_carbon = np.array([1 / r.carbon_emitted if r.carbon_emitted > 0 else 1e6 for r in results])
    inv_cost = np.array([1 / r.total_cost if r.total_cost > 0 else 1e6 for r in results])

    # Apply log scaling
    inv_rebuffers = np.log10(inv_rebuffers)
    inv_carbon = np.log10(inv_carbon)
    inv_cost = np.log10(inv_cost)
...
def simulate_policies(num_segments, policy_dict, verbose=False, plots=False):
    simulation_results = []
    for label, policy in policy_dict.items():
        random.seed(3317303)  #birthdays hehehe, ensures that each simulation run with a different policy has the same stream of randomness generated
        simulator = NetworkSimulator(num_segments, policy)
        res = simulator.simulate()
        ...
        if verbose:
            ...
            print(f"    Average quality (MB/sec): {res.average_quality}")
            print(f"    CO2 emitted (g): {res.carbon_emitted}")
            print(f"    # of rebuffers: {res.num_rebuffer_events}")
            print(f"    Time spent rebuffering (s): {res.total_rebuffer_time}")
